...
func TestGetBoolean(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "non-boolean string rejected",
			args:     map[string]any{"flag": "notabool"},
			param:    "flag",
			required: true,
			want:     false,
			wantErr:  true,
...
func TestGetArrayOfIntegers(t *testing.T) {
	tests := []struct {
		...
		{
			name: "array with numeric string element coerced",
			args: map[string]any{"nums": []any{
		...
		{
			name: "invalid array with non-numeric string",
			args: map[string]any{"nums": []any{
				float64(1), "abc", float64(3),
			}},
			param:    "nums",
			required: true,
			want:     nil,
