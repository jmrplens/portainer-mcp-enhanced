...
Parameters are passed through directly to the underlying handler. Each action accepts the same parameters as its granular tool counterpart — see the [Tools Reference](/portainer-mcp/reference/api-reference/) for parameter details.
...
In granular mode, each tool is registered separately with its own name (e.g., `get_environments`, `create_stack`, etc.). See the [Tools Reference](/portainer-mcp/reference/api-reference/) for the complete list.
